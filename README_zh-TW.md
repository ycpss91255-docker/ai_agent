...
內建 Claude Code、Gemini CLI 與 OpenAI Codex CLI 的 Docker-in-Docker (DinD) AI Agent 開發容器，提供 CPU 與 NVIDIA GPU 兩種版本。以非 root 使用者執行，UID/GID 對應宿主機。
...
- 隔離的 Docker-in-Docker 容器，內建 Claude Code + Gemini CLI + OpenAI Codex CLI
...
- 宿主機上先完成 Claude Code（`claude`）、Gemini CLI（`gemini`）和/或 Codex CLI（`codex`）的 OAuth 登入
...
憑證（`~/.claude`、`~/.gemini`、`~/.codex`）以唯讀方式掛載進容器，首次啟動時複製到資料目錄。後續啟動直接使用既有資料。
...
測試內容：AI 工具可用性、開發工具、系統設定（非 root 使用者、時區、語系），以及確認不必要的工具未安裝（tmux、vim、fzf、terminator）。
...
| `devel` | Claude Code、Gemini CLI、Codex CLI、entrypoint、切換至非 root 使用者 |
